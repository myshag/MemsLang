...
/* === plan-view convex-corner undercut (corner compensation) ============
 * A top-down (x,y) level-set on the wafer surface.  phi < 0 = silicon still
 * present under the mask, phi > 0 = laterally undercut (mask now overhangs
 * void).  The front recedes into the masked silicon at a rate that depends on
 * the in-plane direction of the front normal:
 *
 *   - normal along <110> (x or y axis) -> a {111} sidewall forms, the lateral
 *     undercut is ~zero (r_slow): straight <110> mask edges hold their line;
 *   - normal along <100> (the 45 deg bisector) -> fast {411}/{311}-type planes
 *     appear and undercut quickly (r_fast).
 *
 * A concave silicon corner is bounded by the two slow faces and stays sharp;
 * a *convex* corner sees the fast 45 deg plane and gets beveled -- exactly the
 * convex-corner undercut that compensation structures are designed to defeat.
 * One run advances the fast corner by r_fast um (normalised unit time), so
 * r_fast is the corner undercut budget (~ undercut_ratio * etch depth).
 */
